...
// Drawing primitives.
const lines = topGroup.append("g").selectAll("line")
    .data(links)
    .enter().append("line")
...
let figures = topGroup.append("g");
for (figure of nodes) {
    var element;

    let cssClass = "state " + stateType[figure.type].short;
    const recursive = figure.name === objectName;
    if (recursive)
        cssClass += " recursive";
...
const text = topGroup.append("g").selectAll("text")
    .data(nodes)
    .enter().append("text")
    .attr("x", 0)
    .attr("y", 0)
...
const description = topGroup.append("g").selectAll("description")
    .data(nodes)
    .enter().append("text")
    .attr("x", 0)
    .attr("y", 13)
...
const linkLabels = topGroup.append("g").selectAll("labels")
    .data(links)
    .enter().append("text")
    .attr("x", 0)
    .attr("y", 0)
 ...
 */
function endCoordinate(horizontal, element) {
    if (element.target.type === 13) {
        let line1 = { x1: element.source.x, y1: element.source.y, x2: element.target.x, y2: element.target.y };
        let line2 = {
            x1: element.target.x - element.target.width / 2,
            y1: element.target.y - 25,
            x2: element.target.x + element.target.width / 2,
            y2: element.target.y - 25
        };
...
function resetTransformation() {
    const scale = 0.5 * Math.exp(-nodes.length / 50) + 0.1;
    svg
        .call(zoom.transform, d3.zoomIdentity
            .scale(scale, scale)
