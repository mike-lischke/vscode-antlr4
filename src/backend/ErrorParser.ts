...
export class ErrorParser {
	...
	constructor(private contexts: Set<SourceContext>) {
		contexts.forEach(context => {
			context.diagnostics.length = 0; // Remove all diagnostics we found ourselves. We use ANTLR generated ones now.
		})
	}
		...
		for (let line of lines) {
			if (line.length > 0) {
				...
				if (!matches) {
					...
					// recursion) or non-symbol related problems occurred (e.g. grammar name doesn't match file name).
